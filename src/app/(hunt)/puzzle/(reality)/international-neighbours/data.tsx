...
export const solutionBody = <div className="max-w-3xl space-y-4">
  <div>
    Okay so it’s a word ladder but instead of a letter off they’re a sound off because the answers are all spelled in IPA with one symbol changing between each. It’s also British pronunciation.
  </div>
  <div>
    So the blanks should be filled in this order, with the green symbol being indexed from the highlighted space.
  </div>
  <div className="text-xl">
<span className="text-green-400 font-bold">ɹ</span> 
<span>ɑ</span> 
<span>s</span>
<span>t</span>
<span>ə</span> 
...
<span>s</span>
<span>t</span>
<span className="text-green-400 font-bold">ə</span> 
→   
<span>p</span> 
<span className="text-green-400 font-bold">eɪ</span> 
<span>s</span> 
<span>t</span>
<span>ə</span>  
→ 
...
<div>
(<span className="text-green-400 font-bold">eɪ</span> is a diphthong so it counts as one symbol)
</div>
<div>
  Written in english:
</div>
...
<div>
    (remember it’s British pronunciation!)
</div>
<div>
  The clues are meant to help people guess the words. They are arranged in the order that the indexed symbols should be arranged (so the f from the IPA spelling of fainter will go first because that clue is first, then the eɪ from paster because that clue is second, etc.) 
</div>
<div>
  Then using the highlighted symbol and ordering them based on the clues, you should get: f eɪ v ɹ ə t, aka the British pronunciation of favorite, which they would spell favourite so the answer is <span className="bg-main-text py-0.5 transition-all duration-300 hover:bg-inherit">
  FAVOURITE.
        </span>
